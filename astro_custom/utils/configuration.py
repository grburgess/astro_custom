...
_config_path = Path("~/.config/astro_custom/").expanduser()

_config_name = Path("astro_custom_config.yml")
...
astro_custom_config: astro_customConfig = OmegaConf.structured(astro_customConfig)
...
if _config_file.is_file():
    ...
    astro_custom_config: astro_customConfig = OmegaConf.merge(
        astro_custom_config, _local_config
    )
...
# Write defaults if not
    ...
    with _config_file.open("w") as f:
        ...
        OmegaConf.save(config=astro_custom_config, f=f.name)
...
def show_configuration() -> None:
    ...
    recurse_dict(astro_custom_config, tree)
