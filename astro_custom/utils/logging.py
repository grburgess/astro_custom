...
from .configuration import astro_custom_config
...
astro_custom_dev_log_handler = handlers.TimedRotatingFileHandler(
...
astro_custom_dev_log_handler.setFormatter(_dev_formatter)
astro_custom_dev_log_handler.setLevel(logging.DEBUG)
# now set up the usr log which will save the info

astro_custom_usr_log_handler = handlers.TimedRotatingFileHandler(
    get_path_of_log_file("usr.log"), when="D", interval=1, backupCount=10
)

astro_custom_usr_log_handler.setLevel(logging.INFO)
...
astro_custom_usr_log_handler.setFormatter(_usr_formatter)
...
astro_custom_console_log_handler = RichHandler(
    level=astro_custom_config.logging.level, rich_tracebacks=True, markup=True, console=console
)
astro_custom_console_log_handler.setFormatter(_console_formatter)
...
def silence_warnings():
    ...
    astro_custom_usr_log_handler.addFilter(warning_filter)
    astro_custom_console_log_handler.addFilter(warning_filter)
...
def activate_warnings():
    ...
    astro_custom_usr_log_handler.removeFilter(warning_filter)
    astro_custom_console_log_handler.removeFilter(warning_filter)


def update_logging_level(level):

    astro_custom_console_log_handler.setLevel(level)
...
def setup_logger(name):
    ...
    log.addHandler(astro_custom_dev_log_handler)

    log.addHandler(astro_custom_console_log_handler)

    log.addHandler(astro_custom_usr_log_handler)
